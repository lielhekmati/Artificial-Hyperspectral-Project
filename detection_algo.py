...
def rx(p: float, cube: np.ndarray, m8_cube: np.ndarray, cov: np.ndarray, target_vec: np.ndarray) \
        -> (np.ndarray, np.ndarray):
    """this function implement the RX algorithm on 2 cubes - with and without target.
    params: p, cube, m8, cov, target
    p: the power adding to the target
    cube: the cube to preform on - no subtracting background in assignment(x, not x-m8)
    m8: the m8 calculation to decrease the background effect
    cov: the cube covariance
    target: the index of the wanted target in the cube
    output: matrices after preforming RX - with and without target"""
    no_target_cube = cube - m8_cube
    inv_cov = np.linalg.inv(cov)
    ...
    rx_no_target = np.tensordot(no_target_cube, inv_cov, axes=([2], [1]))
    rx_no_target = np.sum(np.multiply(rx_no_target, no_target_cube), axis=2)

    rx_target = np.tensordot(target_cube, inv_cov, axes=([2], [1]))
    rx_target = np.sum(np.multiply(rx_target, target_cube), axis=2)

    peak_dist = p * np.tensordot(target_vec, inv_cov, axes=([2], [0])).squeeze()
    peak_dist = np.dot(peak_dist, target_vec.squeeze()).squeeze()

    return rx_target, rx_no_target, peak_dist
